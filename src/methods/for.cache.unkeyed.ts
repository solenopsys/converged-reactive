...
class MappedRoot<T = unknown, R = unknown> extends Root {
...
class CacheUnkeyed<T, R> {
	...
	cleanup = (): void => {
		let pooled = 0;
		let poolable = Math.max(
			0,
			this.pooled ? this.poolMaxSize - this.pool.length : 0,
		);
	...
	map = (values: readonly T[]): Resolved<R>[] => {
		...
		let resultsUncached = true; // Whether all results are anew, if so this enables an optimization in Re
		...
		outer: for (let i = 0, l = leftovers.length; i < l; i++) {
			...
			if (pooled && pool.length) {
				...
				mapped.wrap(() => {
					...
					const result = (results[index] = suspended
						? suspense(
								() => suspended.get(),
								() => resolve(fn($value, $index)),
						  )
						: resolve(fn($value, $index)));
