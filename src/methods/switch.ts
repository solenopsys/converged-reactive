...
function _switch<T, R>(
	when: FunctionMaybe<T>,
	values: [...[T, R][], [R]],
	fallback?: undefined,
): ObservableReadonly<Resolved<R>>;
function _switch<T, R, F>(
	when: FunctionMaybe<T>,
	values: [...[T, R][], [R]],
	fallback: F,
): ObservableReadonly<Resolved<R>>;
function _switch<T, R>(
	...
	values: [T, R][],
	fallback?: undefined,
): ObservableReadonly<Resolved<R | undefined>>;
function _switch<T, R, F>(
	when: FunctionMaybe<T>,
	values: [T, R][],
	fallback: F,
): ObservableReadonly<Resolved<R | F>>;
function _switch<T, R, F>(
	when: FunctionMaybe<T>,
	values: ([T, R] | [R])[],
	fallback?: F,
): ObservableReadonly<Resolved<R | F | undefined>> {
	const isDynamic =
		isFunction(when) && !isObservableFrozen(when) && !isUntracked(when);
